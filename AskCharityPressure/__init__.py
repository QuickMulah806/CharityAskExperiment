...
class C(BaseConstants):
    ...
    link1 = "https://www.dropbox.com/s/hjlkgd212c1ehfl/IMG_3670.mp4?raw=1"
...
class Player(BasePlayer):
    ...
    pressure_q = models.IntegerField(
        label="6. With what intensity did you feel social pressure during the experiment?",
        blank=True,
    )
    ...
    controlq_prevent = make_field3(
        label="...the staff member stating that children 'should never die from preventable causes.'"
    )
    controlq_children = models.IntegerField(
...
class Attention1(Page):
    form_model = "player"
    form_fields = ["controlq_fundraising", "controlq_prevent", "controlq_children"]

    @staticmethod
    def before_next_page(player, timeout_happened):
        player.sum_correct = (
            player.controlq_fundraising
            + player.controlq_prevent
            + player.controlq_children
        )
...
class Feedback(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        player.payment_number = random.choices([1, 0], weights=(10, 90), k=1)[0]
        player.randid = get_random_string(8)
